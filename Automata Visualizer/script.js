...
function drawTransitionDiagram(states, alphabet) {
    ...
    // Set canvas dimensions based on viewport width for responsiveness
    const viewportWidth = window.innerWidth;
    canvas.width = viewportWidth > 600 ? 800 : viewportWidth * 0.9;
    ...
    const circleDistance = Math.min(canvas.width, canvas.height) / 2.5;
    const statePositions = {};

    // Draw each state with responsive positioning
    states.forEach((state, index) => {
    ...
    // Handle transitions as per your existing logic
    const table = document.querySelector("table");
